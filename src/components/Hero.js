...
function Hero() {
    ...
    // --- REFACTORED: Typewriter effect using a single state object ---
    ...
    const phrases = ["Web Development", "E-Commerce", "UI/UX Design", "SEO Optimization", "Mobile Apps"];
    ...
    useEffect(() => {
        const currentPhrase = phrases[typewriterState.phraseIndex];
        let timeoutId;

        if (!typewriterState.isDeleting) {
            if (typewriterState.text === currentPhrase) {
                // Start deleting
                ...
                timeoutId = setTimeout(() => {
                    setTypewriterState(prev => ({
                        ...prev,
                        text: currentPhrase.substring(0, prev.text.length + 1),
                        typingSpeed: 150,
                    }));
                }, typewriterState.typingSpeed);
            }
        } else {
            if (typewriterState.text === '') {
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 mt-16">
                <div className="lg:grid lg:grid-cols-2 lg:gap-16 items-center">
                    ...
                    <div className="relative flex justify-center lg:justify-end">
                        ...
                        {/* REMOVED hover:rotate-0 to fix mobile glitching */}
                        <div className="relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl overflow-hidden transform rotate-3 transition-transform duration-500 ease-in-out w-full max-w-md">
                                ...
                                <div className="bg-white/90 rounded-xl overflow-hidden shadow-lg border border-gray-100">
                                    ...
                                    <div className="p-5">
                                        ...
                                        <div className="h-28 bg-gradient-to-r from-indigo-100 to-purple-100 rounded-xl flex items-center justify-center border border-indigo-200">
                                            ...
                                            <span className="text-indigo-700 font-bold text-xl">{typewriterState.text}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
